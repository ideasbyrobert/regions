...
final class WindowManagementLayoutTests: XCTestCase
{
    ...
    func testEveryPresetProducesAVisiblePositiveFrame()
    {
        for preset in LayoutPreset.allCases
        {
            ...
            XCTAssertTrue(screen.visibleFrame.contains(target.frame), preset.title)
    ...
    func testAxisMaximizationPreservesOtherDimension()
    {
        ...
        XCTAssertEqual(widthTarget.frame, CGRect(x: 0, y: 250, width: 1200, height: 400))
        XCTAssertEqual(heightTarget.frame, CGRect(x: 300, y: 0, width: 600, height: 900))
    }
    ...
    func testMoveCommandsPreserveSizeAndReachEdges()
    {
        ...
        XCTAssertEqual(topLeft.frame, CGRect(x: 0, y: 500, width: 600, height: 400))
        XCTAssertEqual(topLeft.edges, [.left, .top])
        XCTAssertEqual(right.frame, CGRect(x: 600, y: 250, width: 600, height: 400))
        XCTAssertEqual(right.edges, [.right])
        XCTAssertEqual(center.frame, CGRect(x: 300, y: 250, width: 600, height: 400))
    }
    ...
    func testNudgeAndResizeCommandsRemainWithinVisibleFrame()
    {
        ...
        XCTAssertEqual(moved.frame, CGRect(x: 340, y: 250, width: 600, height: 400))
        XCTAssertEqual(grown.frame, CGRect(x: 260, y: 210, width: 680, height: 480))
        XCTAssertEqual(narrowed.frame, CGRect(x: 340, y: 250, width: 520, height: 400))
