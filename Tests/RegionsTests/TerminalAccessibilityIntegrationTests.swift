...
final class TerminalAccessibilityIntegrationTests: XCTestCase
{
    func testTerminalWindowsArrangeAtEightyByFortyEightAndRestore() async throws
    {
        ...
        try XCTSkipIf(
            !NSRunningApplication.runningApplications(
                withBundleIdentifier: TerminalWindowSizingService.bundleIdentifier
            ).isEmpty,
        ...
        {
            XCTFail("The fresh Terminal instance did not expose one disposable window")
            return
        }
        ...
        {
            XCTFail("Accessibility did not expose five disposable Terminal windows")
            return
        }
        guard let targetScreen = provider.screen(
        ...
        XCTAssertTrue(arrangedWindows.allSatisfy
        {
    ...
    private func createTerminalWindows(count: Int) throws
    {
        ...
        if let errorInformation
        {
            ...
            throw NSError(
                ...
                code: (errorInformation[NSAppleScript.errorNumber] as? NSNumber)?.intValue ?? 0,
    ...
    {
#if GRIDWINDOWMANAGER_RUN_TERMINAL_TESTS
        true
#else
        false
#endif
