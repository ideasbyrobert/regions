...
final class EdgeSnapResolverTests: XCTestCase
{
    ...
    {
        XCTAssertEqual(
            resolver.zone(at: CGPoint(x: 720, y: 899), on: screen, threshold: 24),
            .fill
        )
    }
    ...
    func testCornersResolveBeforeStraightEdges()
    {
        ...
        XCTAssertEqual(
            resolver.zone(at: CGPoint(x: 1439, y: 1), on: screen, threshold: 24),
            .bottomRightQuarter
        )
    }
    ...
    func testStraightSideEdgesResolveToHalves()
    {
        ...
        XCTAssertEqual(
            resolver.zone(at: CGPoint(x: 1439, y: 450), on: screen, threshold: 24),
            .rightHalf
        )
    }
    ...
    {
        XCTAssertNil(
            resolver.zone(at: CGPoint(x: 720, y: 450), on: screen, threshold: 24)
        )
    }
}
