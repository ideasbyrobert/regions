...
final class BalancedWindowArrangementServiceTests: XCTestCase
{
    ...
    {
        let original = makeWindows(
            count: 5, size: CGSize(width: 160, height: 120))
        ...
        XCTAssertEqual(
            result,
            .arranged(
                windowCount: 5, usedBestEffort: false, terminalSized: false)
        )
    ...
    {
        let original = makeWindows(
            count: 2, size: CGSize(width: 180, height: 100))
        let manager = FakeWindowManager(
            windows: original,
            captureQueue: [original]
        )
    ...
    func testTerminalSizingFailureRollsBackDimensionsAndFrames() async
    {
        ...
        terminal.failNextApply(
            with: .scriptFailure(code: -1708, message: "Rejected"))
        let service = makeService(manager: manager, terminal: terminal)
    ...
    {
        let original = makeWindows(
            count: 8, size: CGSize(width: 180, height: 100))
    ...
    {
        let original = makeWindows(
            count: 5, size: CGSize(width: 180, height: 100))
        let resized = replacingSizes(
            in: original,
    ...
    {
        let original = makeWindows(
            count: 9, size: CGSize(width: 160, height: 120))
        let manager = FakeWindowManager(
            windows: original,
            captureQueue: [original]
        )
    ...
    func testTerminalWindowSetMismatchFailsBeforeSizing() async
    {
        ...
        XCTAssertEqual(
            result,
            .failed(
                .terminalWindowCountMismatch(accessibility: 2, automation: 1))
        )
        XCTAssertEqual(currentWindows, original)
        XCTAssertTrue(terminal.appliedStates.isEmpty)
        XCTAssertEqual(transactionCount, 0)
