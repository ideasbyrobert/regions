...
DEVELOPMENT_TEAM_IDENTIFIER="${REGIONS_DEVELOPMENT_TEAM:-X87D35HM5V}"
DEVELOPMENT_SIGNING_IDENTITY="${REGIONS_DEVELOPMENT_IDENTITY:-Apple Development}"
...
case "$MODE" in
    ...
    --live-accessibility|live-accessibility)
        XCODEBUILD_ARGUMENTS+=(
            'SWIFT_ACTIVE_COMPILATION_CONDITIONS=$(inherited) REGIONS_RUN_AX_TESTS'
            "-only-testing:RegionsTests/AccessibilityIntegrationTests"
        )
        ;;
    --live-app-accessibility|live-app-accessibility)
        XCODEBUILD_ARGUMENTS+=(
            'SWIFT_ACTIVE_COMPILATION_CONDITIONS=$(inherited) REGIONS_RUN_APP_AX_UI_TESTS'
            "-only-testing:RegionsUITests/LiveAccessibilityUITests"
        )
        ;;
    --live-terminal|live-terminal)
        XCODEBUILD_ARGUMENTS+=(
            'SWIFT_ACTIVE_COMPILATION_CONDITIONS=$(inherited) REGIONS_RUN_TERMINAL_TESTS'
