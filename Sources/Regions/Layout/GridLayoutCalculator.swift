...
struct GridLayoutCalculator: LayoutCalculating
{
    ...
    {
        let availableFrame = availableFrame(
            screen: screen,
            spacing: spacing
        )
        var frame: CGRect
        if placement.position == .fill
        {
            frame = availableFrame
        } else if let size = placement.size
        {
            let fraction = CGFloat(size.fraction)
            switch placement.orientation
            {
            case .landscape:
                let width = availableFrame.width * fraction
                frame = CGRect(
                    x: horizontalOrigin(
            ...
            case .portrait:
                let height = availableFrame.height * fraction
                frame = CGRect(
        ...
        {
            frame = bounded(currentWindowFrame, within: availableFrame)
            switch placement.orientation
            {
            case .landscape:
                frame.origin.x = horizontalOrigin(
                    position: placement.position,
                    dimension: frame.width,
                    in: availableFrame
                )
                frame.origin.y = availableFrame.midY - frame.height / 2
            case .portrait:
                frame.origin.x = availableFrame.midX - frame.width / 2
        ...
        frame = resized(
            frame,
            widthDelta: CGFloat(placement.widthDelta),
            heightDelta: CGFloat(placement.heightDelta)
        )
        frame.origin.x += CGFloat(placement.horizontalOffset)
        frame.origin.y += CGFloat(placement.verticalOffset)
    ...
    ) -> CGFloat
    {
        ...
        case .leading:
            frame.minX
        case .center, .fill:
            frame.midX - dimension / 2
        case .trailing:
            frame.maxX - dimension
        }
    }
    ...
    ) -> CGFloat
    {
        ...
        {
        case .leading:
            frame.maxY - dimension
        case .center, .fill:
            frame.midY - dimension / 2
        case .trailing:
            frame.minY
        }
    }
    ...
    ) -> LayoutTarget
    {
        ...
        case .maximizeWidth:
            let availableFrame = availableFrame(
                screen: screen, spacing: spacing)
            let boundedFrame = bounded(
                currentWindowFrame, within: availableFrame)
            return target(
                frame: CGRect(
                    x: availableFrame.minX,
                    y: boundedFrame.minY,
    ...
    ) -> LayoutTarget
    {
        ...
        case .bottomLeftCorner:
            frame.origin = CGPoint(
                x: availableFrame.minX, y: availableFrame.minY)
            edges = [.left, .bottom]
    ...
    ) -> LayoutTarget
    {
        let availableFrame = availableFrame(screen: screen, spacing: spacing)
        var frame = bounded(currentWindowFrame, within: availableFrame)
        let resolvedAmount = min(
            max(amount, 1), max(availableFrame.width, availableFrame.height))
        switch adjustment
        {
        case .moveLeft:
        ...
        case .widen:
            frame = resized(
                frame, widthDelta: resolvedAmount * 2, heightDelta: 0)
        case .narrow:
            frame = resized(
                frame, widthDelta: -resolvedAmount * 2, heightDelta: 0)
        case .taller:
            frame = resized(
                frame, widthDelta: 0, heightDelta: resolvedAmount * 2)
        case .shorter:
            frame = resized(
                frame, widthDelta: 0, heightDelta: -resolvedAmount * 2)
        }
    ...
    {
        let gap = resolvedSpacing(
            spacing, columns: columns, rows: rows, screen: screen)
        let scale = max(screen.backingScaleFactor, 1)
        let visibleFrame = screen.visibleFrame
        let cellWidth =
            (visibleFrame.width - gap * CGFloat(columns + 1)) / CGFloat(columns)
        let cellHeight =
            (visibleFrame.height - gap * CGFloat(rows + 1)) / CGFloat(rows)

        let minimumX =
            visibleFrame.minX + gap + CGFloat(minimumColumn) * (cellWidth + gap)
        let maximumX =
            visibleFrame.minX + gap + CGFloat(maximumColumn + 1) * cellWidth
            + CGFloat(maximumColumn) * gap
        let maximumY =
            visibleFrame.maxY - gap - CGFloat(minimumRow) * (cellHeight + gap)
        let minimumY =
            visibleFrame.maxY - gap - CGFloat(maximumRow + 1) * cellHeight
            - CGFloat(maximumRow) * gap
    ...
    private func availableFrame(screen: ScreenSnapshot, spacing: CGFloat)
        -> CGRect
    {
    ...
    private func bounded(_ frame: CGRect, within availableFrame: CGRect)
        -> CGRect
    {
    ...
    private func edges(for frame: CGRect, within availableFrame: CGRect)
        -> LayoutEdges
    {
        ...
        let maximumHorizontal = max(
            ...
            (
                screen.visibleFrame.width
                    - CGFloat(columns) * minimumCellDimension
            )
                / CGFloat(columns + 1)
        )
        let maximumVertical = max(
            0,
            (screen.visibleFrame.height - CGFloat(rows) * minimumCellDimension)
                / CGFloat(rows + 1)
        )
        return min(
            min(max(requestedSpacing, 0), maximumHorizontal), maximumVertical)
    }
