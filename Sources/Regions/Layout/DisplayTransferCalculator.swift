...
struct DisplayTransferCalculator: Sendable
{
    ...
    ) -> LayoutTarget
    {
        ...
        let destinationBounds = inset(
            destination.visibleFrame, spacing: spacing)
        let widthRatio =
            sourceBounds.width > 0 ? currentFrame.width / sourceBounds.width : 1
        let heightRatio =
            sourceBounds.height > 0
            ? currentFrame.height / sourceBounds.height : 1
        let xRatio =
            sourceBounds.width > currentFrame.width
            ? (currentFrame.minX - sourceBounds.minX)
                / (sourceBounds.width - currentFrame.width)
            : 0.5
        let yRatio =
            sourceBounds.height > currentFrame.height
            ? (currentFrame.minY - sourceBounds.minY)
                / (sourceBounds.height - currentFrame.height)
            : 0.5
        let width = min(
            destinationBounds.width, destinationBounds.width * widthRatio)
        let height = min(
            destinationBounds.height, destinationBounds.height * heightRatio)
        let x =
            destinationBounds.minX + clamped(xRatio)
            * max(0, destinationBounds.width - width)
        let y =
            destinationBounds.minY + clamped(yRatio)
            * max(0, destinationBounds.height - height)
