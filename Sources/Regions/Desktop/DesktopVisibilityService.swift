...
final class DesktopVisibilityService: ObservableObject
{
    ...
    func showDesktop() -> DesktopVisibilityResult
    {
        ...
        for application in candidates
        {
            ...
            else
            {
                ...
                if !rollbackFailures.isEmpty
                {
                    ...
                    isDesktopShown = true
                    logger.error(
                        "Show Desktop failed and retained retryable restore state"
                    )
                    return .failed(
                        "An application could not be hidden, and some hidden applications still need restoration."
                    )
                }
                logger.error("Show Desktop failed and reverted hidden applications")
                return .failed("An application could not be hidden. No desktop state was saved.")
            }
        ...
        isDesktopShown = true
        logger.info(
            "Showed desktop by hiding \(hidden.count, privacy: .public) applications"
        )
    ...
    func restoreDesktop() -> DesktopVisibilityResult
    {
        ...
        logger.info(
            "Restored desktop visibility for \(restoredCount, privacy: .public) applications"
        )
