...
actor AccessibilityWindowActor: WindowManaging
{
    ...
    ) async -> WindowCaptureResult
    {
        ...
        guard
            let window = application.copiedElement(
        ...
        logger.info(
            """
            Captured focused window for process \
            \(processIdentifier, privacy: .public)
            """
        )
        return .captured(
            registeredSnapshot(
    ...
    ) async -> Result<[ManagedWindowSnapshot], MoveFailure>
    {
        ...
        AXUIElementSetMessagingTimeout(application, 0.5)
        guard
            let windows = application.copiedElements(
        ...
        logger.info(
            """
            Captured \(snapshots.count, privacy: .public) standard windows \
            for process \(processIdentifier, privacy: .public)
            """
        )
    ...
    ) async -> WindowCaptureResult
    {
        ...
        let accessibilityPoint = converter.accessibilityPoint(
    ...
    ) async -> WindowCaptureResult
    {
        ...
        return .captured(
            ManagedWindowSnapshot(
                ...
                frame: converter.appKitFrame(
                    fromAccessibilityFrame: accessibilityFrame
                ),
                isResizable: record.element.isAttributeSettable(
    ...
    ) async -> MoveResult
    {
        ...
        for request in requests
        {
            ...
            case .moved(let actualFrame):
                ...
                else
                {
                    ...
                    {
                        _ = await restoreFrames(
                            rollbackStates, converter: converter)
                    }
                    return .failed(.staleWindow)
                }
                rollbackStates.append(
                    WindowFrameState(
                        token: request.token,
                        frame: previousFrame
                    ))
        ...
        logger.info(
            """
            Applied batch of \(changes.count, privacy: .public) windows in \
    ...
    ) async -> WindowOperationResult
    {
        ...
        case .minimize:
            guard
                record.element.isAttributeSettable(
                    kAXMinimizedAttribute as CFString)
            else
            {
                return .failed(.windowActionUnavailable)
            }
            error = record.element.setBoolean(
                true, for: kAXMinimizedAttribute as CFString)
        case .zoom:
            guard
                let button = record.element.copiedElement(
                    for: kAXZoomButtonAttribute as CFString)
            else
            {
                return .failed(.windowActionUnavailable)
            }
        ...
        case .toggleFullScreen:
            ...
            let isFullScreen =
                record.element.copiedBoolean(for: attribute) == true
            error = record.element.setBoolean(!isFullScreen, for: attribute)
        case .close:
            guard
                let button = record.element.copiedElement(
                    for: kAXCloseButtonAttribute as CFString)
            else
            {
                return .failed(.windowActionUnavailable)
            }
        ...
        else
        {
            logger.error(
                """
                Window action failed with AX error \
                \(error.rawValue, privacy: .public)
                """
            )
            ...
            let previousFrame
        {
            ...
            if let managedFrame = readAppKitFrame(
                record.element, converter: converter),
        ...
        logger.info(
            "Window action completed: \(action.title, privacy: .public)")
        return .completed
    }
    ...
    ) async -> Result<WindowArrangementTransaction, MoveFailure>
    {
        ...
        for change in transaction.changes
        {
            guard let record = records[change.token.id],
                let currentFrame = readAppKitFrame(
                    record.element, converter: converter)
            else
            {
                return .failure(.staleWindow)
            ...
            guard
                approximatelyEqual(
                    currentFrame, change.managedFrame, tolerance: 2)
            else
            {
    ...
    ) async -> MoveResult
    {
        ...
        for state in states
        {
            ...
            currentStates.append(
                WindowFrameState(token: state.token, frame: currentFrame))
        }
        ...
        for state in states
        {
            ...
            let resize = record.element.isAttributeSettable(
                kAXSizeAttribute as CFString)
            let error = set(
            ...
            guard error == .success,
                let restoredFrame = readAppKitFrame(
                    record.element, converter: converter)
            else
            {
                rollback(currentStates, converter: converter)
        ...
        logger.info(
            """
            Restored \(restoredFrames.count, privacy: .public) windows in \
            \(start.duration(to: .now), privacy: .public)
            """
        )
        return usedBestEffort ? .bestEffort(firstFrame) : .moved(firstFrame)
    ...
    ) async -> (result: MoveResult, previousFrame: CGRect?)
    {
        ...
        let isResizable = record.element.isAttributeSettable(
            kAXSizeAttribute as CFString)
        ...
        if initialError != .success
        {
            ...
            logger.error(
                """
                Initial window move failed with AX error \
                \(initialError.rawValue, privacy: .public)
                """
            )
            return (
                .failed(.accessibilityFailure(initialError.rawValue)),
                previousFrame
            )
        }
        ...
        if alignmentError != .success
        {
            ...
            logger.error(
                """
                Aligned window move failed with AX error \
                \(alignmentError.rawValue, privacy: .public)
                """
            )
            return (
                .failed(.accessibilityFailure(alignmentError.rawValue)),
                previousFrame
            )
        }
            ...
            }) ?? true
        {
            ...
            if retryError != .success
            {
                ...
                logger.error(
                    """
                    Window move retry failed with AX error \
                    \(retryError.rawValue, privacy: .public)
                    """
                )
                return (
                    .failed(.accessibilityFailure(retryError.rawValue)),
                    previousFrame
                )
            }
        ...
        let exact = approximatelyEqual(
            actualFrame, target.frame, tolerance: tolerance)
        logger.info(
            """
            Window move completed with exact target: \
    ...
    )
    {
        for state in states
        {
            ...
            _ = set(
                ...
                on: record.element,
                converter: converter,
                resize: record.element.isAttributeSettable(
                    kAXSizeAttribute as CFString)
            )
        }
    }
    ...
    private func bringToFront(_ record: AccessibilityWindowRecord) -> AXError
    {
        ...
        if application.isAttributeSettable(
    ...
    {
        element.copiedString(for: kAXRoleAttribute as CFString) == kAXWindowRole
            as String
            && element.copiedString(for: kAXSubroleAttribute as CFString)
    ...
    private func standardWindowAncestor(of element: AXUIElement) -> AXUIElement?
    {
        ...
        for _ in 0..<16
        {
            ...
            current = candidate.copiedElement(
                for: kAXParentAttribute as CFString)
        }
        return nil
    }
    ...
    ) -> ManagedWindowSnapshot
    {
        ...
        })
        {
            return ManagedWindowSnapshot(
                ...
                isResizable: element.isAttributeSettable(
                    kAXSizeAttribute as CFString
                )
            )
        }
    ...
    {
        guard
            let position = element.copiedPoint(
                for: kAXPositionAttribute as CFString),
    ...
    {
        let accessibilityFrame = converter.accessibilityFrame(
            fromAppKitFrame: appKitFrame)
        if resize
        {
            let firstSizeError = element.setSize(
                accessibilityFrame.size, for: kAXSizeAttribute as CFString)
            if firstSizeError != .success
            {
                return firstSizeError
            }
        }
        let positionError = element.setPoint(
            accessibilityFrame.origin, for: kAXPositionAttribute as CFString)
    ...
    private func constrainedFrame(_ actualFrame: CGRect, target: LayoutTarget)
        -> CGRect
    {
        ...
        origin.x = min(
            max(
                origin.x,
                target.visibleFrame.minX - actualFrame.width + horizontalReach),
            target.visibleFrame.maxX - horizontalReach
        )
        origin.y = min(
            max(
                origin.y,
                target.visibleFrame.minY - actualFrame.height + verticalReach),
    ...
    private func approximatelyEqual(
        _ first: CGRect, _ second: CGRect, tolerance: CGFloat
    ) -> Bool
    {
