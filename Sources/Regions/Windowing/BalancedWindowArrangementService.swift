...
final class BalancedWindowArrangementService
{
    ...
    ) async -> WindowArrangementResult
    {
        ...
        let isTerminal = bundleIdentifier == TerminalWindowSizingService.bundleIdentifier
        ...
        if isTerminal
        {
            ...
            {
                return .failed(.terminal(.scriptFailure(
                    code: 0,
            ...
            {
                return .failed(.terminalWindowCountMismatch(
            ...
            catch
            {
                ...
                return .failed(rollbackFailure ?? .terminal(.scriptFailure(
                    code: 0,
                    message: error.localizedDescription
                )))
            }
            ...
            guard originalWindows.allSatisfy(
            {
    ...
    ) async -> WindowArrangementResult
    {
        ...
        if let terminalState = transaction.terminalState
        {
            ...
            do
            {
                ...
                guard normalized(currentStates) == normalized(terminalState.managed)
                else
                {
            ...
            catch let failure as TerminalWindowSizingError
            {
                let rollbackFailure = await rollbackExternalState(
                    ...
                    {
                        WindowFrameState(token: $0.token, frame: $0.managedFrame)
                    },
                    converter: converter
                )
                return .failed(rollbackFailure ?? .terminal(failure))
    ...
    ) async -> WindowArrangementFailure?
    {
        ...
        case .moved:
            ...
            return nil
        case .bestEffort:
            let detail = terminalFailure.map
            {
