...
final class DisplayWindowManagementService
{
    ...
    ) async -> DisplayWindowManagementResult
    {
        ...
        {
            guard let source = displayProvider.screen(
            ...
            requests.append(WindowPlacementRequest(
                token: window.token,
                target: target,
        ...
        logger.info(
            "Transferred \(requests.count, privacy: .public) app windows in \(start.duration(to: .now), privacy: .public)"
        )
        switch result
        {
        case .moved:
    ...
    ) async -> DisplayWindowManagementResult
    {
        ...
        for window in windows
        {
            guard let screen = displayProvider.screen(containing: window.frame, in: screens),
                  let sourceIndex = screens.firstIndex(where: { $0.displayID == screen.displayID })
            else
            {
                continue
            }
