...
enum WindowArrangementFailure: Error, Equatable, Sendable
{
    ...
    var message: String
    {
        ...
        case .tooManyWindows(let count):
            return
                "Arrangement is unavailable for \(count) windows. Maximum: 8."
        ...
        case .terminalWindowCountMismatch(let accessibility, let automation):
            return
                "Terminal exposed \(accessibility) manageable windows and "
                + "\(automation) automation windows. No windows were changed."
        case .doesNotFit(let required, let available):
            return
                "Exact 80 × 48 windows require "
                + "\(Int(ceil(required.width))) × "
                + "\(Int(ceil(required.height))) pt; this display provides "
                + "\(Int(floor(available.width))) × "
                + "\(Int(floor(available.height))) pt. "
                + "No windows were changed."
        case .rollbackFailed(let detail):
            return
                "The arrangement failed and could not be completely "
                + "restored: \(detail)"
        }
    }
}
