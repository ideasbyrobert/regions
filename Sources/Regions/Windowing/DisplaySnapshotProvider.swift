...
struct DisplaySnapshotProvider
{
    ...
    func screen(containing windowFrame: CGRect, in screens: [ScreenSnapshot])
        -> ScreenSnapshot?
    {
        ...
        {
            left, right in
            intersectionArea(windowFrame, left.frame)
                < intersectionArea(windowFrame, right.frame)
        }
        if let intersecting,
            intersectionArea(windowFrame, intersecting.frame) > 0
        {
            return intersecting
        }
        ...
        {
            left, right in
            squaredDistance(from: windowFrame, to: left.frame)
                < squaredDistance(from: windowFrame, to: right.frame)
        }
    }

    func screen(containing point: CGPoint, in screens: [ScreenSnapshot])
        -> ScreenSnapshot?
    {
        screens.first
        {
    ...
    ) -> ScreenSnapshot?
    {
        ...
        case .previous:
            let index =
                currentIndex == 0 ? orderedScreens.count - 1 : currentIndex - 1
            return orderedScreens[index]
        case .next:
            let index =
                currentIndex == orderedScreens.count - 1 ? 0 : currentIndex + 1
            return orderedScreens[index]
        }
    }
    ...
    private func squaredDistance(from first: CGRect, to second: CGRect)
        -> CGFloat
    {
