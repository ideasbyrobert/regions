...
final class TerminalWindowSizingService: ObservableObject, TerminalWindowSizing
{
    ...
    @Published private(set) var authorizationState = TerminalAutomationAuthorizationState.notRequested
    ...
    func captureStates() throws -> [TerminalWindowState]
    {
        ...
        tell application id "com.apple.Terminal"
            ...
            repeat with terminalWindow in windows
                ...
                set end of stateValues to number of columns of selected tab of terminalWindow
                set end of stateValues to number of rows of selected tab of terminalWindow
            end repeat
            return stateValues
        end tell
        """
    ...
    func apply(_ states: [TerminalWindowState]) throws
    {
        ...
        let commands = states.map
        {
            state in
            """
                set number of columns of selected tab of window id \(state.windowIdentifier) to \(state.columns)
                set number of rows of selected tab of window id \(state.windowIdentifier) to \(state.rows)
            """
        }
        .joined(separator: "\n")
        let source = """
    ...
    static func managedStates(from states: [TerminalWindowState]) -> [TerminalWindowState]
    {
        states.map
        {
            TerminalWindowState(
    ...
    static func normalized(_ states: [TerminalWindowState]) -> [TerminalWindowState]
    {
        states.sorted
        {
    ...
    static func states(from descriptor: NSAppleEventDescriptor) throws -> [TerminalWindowState]
    {
    ...
    private func execute(_ source: String) throws -> NSAppleEventDescriptor
    {
        ...
        let executionResult: NSAppleEventDescriptor? = script.executeAndReturnError(
            &errorInformation
        )
        guard let result = executionResult
        else
        {
            let code = (errorInformation?[NSAppleScript.errorNumber] as? NSNumber)?.intValue ?? 0
            let message = errorInformation?[NSAppleScript.errorMessage] as? String
                ?? "Unknown error"
            ...
            throw TerminalWindowSizingError.scriptFailure(code: code, message: message)
        }
        return result
    }
